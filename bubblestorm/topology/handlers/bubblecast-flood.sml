...
fun handleBubblecast
   ...
   let
      ...
      val numEdges = Iterator.length edges
      
      val bubbleLen = bubbleStop - bubbleStart
      fun forward (i, neighbour) =
         let      
            val subBubbleStart = bubbleStart + (i*bubbleLen div numEdges) 
            val subBubbleStop  = bubbleStart + ((i+1)*bubbleLen div numEdges)
            
            val subSliceStart = Int.max (sliceStart, subBubbleStart)
            val subSliceStop  = Int.min (sliceStop,  subBubbleStop)
            
            val () = Log.logExt (Log.DEBUG, method, fn () =>
               ("forward: "
               ^ ", subBubbleStart=" ^ (Int.toString subBubbleStart) ^ ", subBubbleStop=" ^ (Int.toString subBubbleStop)
         ...
         in
            ...
            in
               bubblecast (priority, reliability) typ seed subBubbleStart subBubbleStop subSliceStart subSliceStop payload
            end      
         end
   in
      Iterator.appi forward edges 
   end
